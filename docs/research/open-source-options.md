...
|---|---|---|---|
| `tauri-apps/tauri` | desktop packaging and native runtime | adopt | Apache-2.0; active official project; fits a compact local desktop app with a web UI and native core. |
| `tauri-apps/plugins-workspace` (mirrored as individual `tauri-plugin-*` repositories) | dialog, autostart, store and supported OS integrations | evaluate per capability | use official maintained plugins only where they do not bypass grant enforcement. Dialog is likely needed; autostart is M5; store is UI preferences only, never source grants or evidence indexes. |
| `electron/electron` | alternative desktop runtime | fallback only | mature MIT alternative, but not selected unless Tauri blocks platform packaging or required native behavior. |
| `WiseLibs/better-sqlite3` | synchronous Node SQLite | reject for selected core | a good Node option, but the selected native core uses Rust SQLite to keep filesystem policy and scanner close together. |
