  ...
  return (
    <main className="cockpit-shell">
      ...
      <section id="workspace" className="workspace" aria-labelledby="page-title" tabIndex={-1}>
        ...
        {activeView === 'overview' ? <>
          {loading ? <StatePanel title="正在加载本地证据" detail="正在检查本地服务并读取仪表盘。" /> : hasError ? <StatePanel title="本地数据不可用" detail="一个或多个本地只读端点加载失败。"><button className="text-button" type="button" onClick={() => void refresh()}>重试</button></StatePanel> : !observedData ? <StatePanel title="尚无已索引的证据" detail="本地服务已连接，但仪表盘暂无可见证据。先在来源中心授权一个数据来源并执行扫描，此视图便会填充真实工作记录。"><button className="text-button text-button--primary" type="button" onClick={() => setActiveView('sources')}>前往来源中心</button></StatePanel> : <section id="data-state" className="dashboard-content" aria-label="仪表盘数据"><div className="section-heading"><div><p className="eyebrow">观测到的仪表盘数据</p><h2>当前证据</h2></div>{runtime.refreshedAt ? <time dateTime={runtime.refreshedAt.toISOString()}>刚刚更新</time> : null}</div><div className="metrics-grid">{metrics.map((metric) => <article className="metric-card" key={metric.label}><p>{metric.label}</p><strong>{metric.value}</strong></article>)}</div></section>}
